...
import { ItemGroup, ItemContent, ItemTitle, ItemDescription, ItemActions, Item } from '@/components/ui/item'
...
export const SearchDialog = () => {
    ...
    return <Dialog open={searchDialogOpen} onOpenChange={setSearchDialogOpen}>
        <DialogTrigger asChild>
            <Button variant='ghost' className='me:auto max-lg:size-9 lg:bg-secondary dark:lg:bg-secondary/50' onClick={() => setSearchDialogOpen((prev) => !prev)}>
        ...
        <DialogContent className='p-0 bg-card gap-0' showCloseButton={false}>
            <DialogHeader className='sr-only'>
                ...
                <DialogDescription>Find weather in your city</DialogDescription>
            </DialogHeader>
            ...
            <ItemGroup className='min-h-80 p-2'>
                ...
                {results.map(({name, lat, lon, state, country}) => (
                    <Item key={name + lat + lon} size='sm' className='realtive p-2'>
                        <ItemContent>
                            <ItemTitle>{name}</ItemTitle>
                            <ItemDescription>{state ? state + ', ': ''}{country}</ItemDescription>
                        </ItemContent>
                        <ItemActions>
                            <DialogClose asChild>
                                <Button variant='ghost' size='icon' className='after:absolute after:inset-0' onClick={() => {}}>
                                    <MapPinnedIcon/>
                                </Button>
                            </DialogClose>
                        </ItemActions>
